...
exports.run = async (client, message, cmd, args, level) => { // eslint-disable-line no-unused-vars
    ...
    try {
        ...
        if (searchTerm.length < 2) return await guildMessage.edit(`${message.author}, please use 2 or more letters to search for characters, ships, I don't want to spam your channel with every character.`);
        ...
        if (lookup.length == 0) return await guildMessage.edit(`${message.author}, I can't find any characters or factions with __${searchTerm}__ in it.`).then(client.cmdError(message, cmd));
        ...
        for (var k = 0; k < lookup.length; k++) {
            ...
            if (guildData[character] === undefined) {
                await guildMessage.edit(`${message.author}, I don't think anyone in ${guildName} has __${lookup[k].name}__.`);
                continue;
            }
